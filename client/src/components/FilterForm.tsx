...
const FilterForm: React.FC<Props> = ({ users, setUsers, originalUsers }) => {
  ...
  const handleSelectFilters = (e: any) => {
    ...
    const filteredUsers = [...originalUsers].filter((user: any) => {
      let newFilters = { ...filters, [target]: value };
      let isMatch =
        user.service?.toLowerCase() ===
          (newFilters.service?.toLowerCase() || user.service?.toLowerCase()) &&
        user.city?.toLowerCase() ===
          (newFilters.city?.toLowerCase() || user.city?.toLowerCase()) &&
        user.sector?.toLowerCase() ===
          (newFilters.sector?.toLowerCase() || user.sector?.toLowerCase()) &&
        user.minPrice >= (newFilters.min_price || user.minPrice) &&
        user.maxPrice <= (newFilters.max_price || user.maxPrice);
  ...
  const handlePriceFilters = (e: any) => {
    let target =
      e.target.getAttribute("id") === "min_price" ? "minPrice" : "maxPrice";
    const value = (e.target as HTMLInputElement).value;

    const filteredUsers = [...originalUsers].filter((user: any) => {
      if (target === "minPrice") {
        return parseInt(user.minPrice) >= parseInt(value);
      }
      return parseInt(user.maxPrice) <= parseInt(value);
    });
    ...
    <div className="flex flex-col mb-20">
      <div className="flex justify-between">
        ...
        <button
          ...
          className="font-semibold text-red-500 text-sm outline-none"
        >
          Supprimer les filtres
        </button>
      </div>
      <hr />
      <form className="flex flex-col items-start md:flex-row gap-4 justify-between md:items-center mt-6 ">
