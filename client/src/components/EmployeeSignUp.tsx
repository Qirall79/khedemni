...
import signUpEmployee from "../utils/signUpEmployee";
...
const EmployeeSignUp: React.FC = () => {
  ...
  // Show/hide the service field in case the user chose "autre"
  const handleSelect = (e: any) => {
    const choice = e.target.value;
    const input = document.querySelector("#service-field");
    if (choice === "autre") {
  ...
  const uploadImage = async () => {
    const fileInput: any = document.querySelector("#photo")!;
    const image: any = fileInput!.files[0]!;

    if (!image) return;
    const formData = new FormData();
    formData.append("file", image);
    formData.append("upload_preset", "walid-khdemni");
  ...
  const handleSignUp = async (data: IData) => {
    const activitySelect = (
      document.querySelector("#services") as HTMLInputElement
    ).value;
    ...
    checkBoxes.forEach((box: any) => {
      if (box.checked) {
        availabilityDays.push(box.value);
      }
    });
    if (availabilityDays.length === 0) {
      setCheckBoxMessage("Veuillez choisir au moins 1 jour");
      return;
    }
    ...
    // set service
    data.service = data.service || activitySelect;

    await signUpEmployee(data, setResponseMessage, employeeDispatch);
  };
      ...
      >
        ...
        >
          ...
          <div
            ...
            id="service-field"
          >
            <label htmlFor="service">Quel est votre service ?</label>
            <input
              className="w-full h-8 rounded-sm text-black outline-none pl-2"
              type="text"
              name="service"
          ...
          <div className="form-group w-full flex flex-col gap-y-2">
            <label htmlFor="min_price">Prix minimum (DH)</label>
            <input
              {...register("min_price", {
                required: "Veuillez entrez votre prix minimum",
              })}
              className="w-full h-8 rounded-sm text-black outline-none pl-2"
              type="number"
              name="min_price"
          ...
          <div className="form-group w-full flex flex-col gap-y-2">
            <label htmlFor="max_price">Prix maximum (DH)</label>
            <input
              {...register("max_price", {
                required: "Veuillez entrez votre prix maximum",
              })}
              className="w-full h-8 rounded-sm text-black outline-none pl-2"
              type="number"
              name="max_price"
          ...
          <div className="form-group w-full flex flex-col gap-y-2">
            ...
            <input
              className="w-full h-8 rounded-sm outline-none pl-2"
              type="file"
              name="photo"
              id="photo"
            />
          ...
          <div className="form-group w-full flex flex-col gap-y-2">
            <label htmlFor="note">Vous avez quelques chose à ajouter ?</label>
            <input
              {...register("note")}
              className="w-full h-8 rounded-sm text-black outline-none pl-2"
              type="text"
